...
(function () {
  ...
  RegisterService.$inject = ['$cordovaSQLite','session'];

  /**
   * @namespace DailyExpensesService
   * @memberOf DailyExpenses
   */
  function RegisterService($cordovaSQLite,session) {
    var service = {
...
function __ConsultarTarjetaPA(tarjeta){
  ...
  return $cordovaSQLite.execute(db, 'select valcuota from tarjetas where contarjeta=? ',[tarjeta]).then(
    ...
    function __updateTarjetaEliminar(data){
            ...
            return $cordovaSQLite.execute(db, 'update tarjetas set finalizada=?, fecultpago=?, fecfinal=?, valpagado=((select valpagado from tarjetas where contarjeta=?)-(select valor from detalletarjeta where contarjeta=? and fecha=?)) where contarjeta=? ',[0, null, null,data.contarjeta,data.contarjeta,fecha,data.contarjeta]).then(
                    function(res) {
